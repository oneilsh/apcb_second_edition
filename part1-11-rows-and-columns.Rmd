...
In order to get the correct number of sequences that had matches to others, we need to remove any duplicates that might still be found in the first column. We can do this by adding a `sort -k1,1d -u`, for a final answer of 2,989.
...
> grep -v '#' | \
> awk '{print NR,$0}'</b>
1 YAL068C	YGL261C	121	121	121	1	121	1	121	3.27e-86
2 YAL068C	YAL068C	121	121	121	1	121	1	121	3.27e-86
3 YAL067W-A	YAL067W-A	76	76	76	1	76	1	76	1.48e-50
...
</code></pre>

`awk` can also execute statements conditionally; the syntax is `<logical expression> {<statements to execute>}`. Additionally, if a column contains numeric values, `awk` can work with them as such, and `awk` even understands scientific notation. Here's an example where only lines with HSP *E* values (the tenth column in our example) of less than `1e-10` are printed.
...
good YAL018C YOL048C 2.73e-25
great YAL017W YAL017W 0.0
great YAL017W YOL045W 0.0
great YAL016W YAL016W 0.0
great YAL015C YAL015C 0.0
great YAL015C YOL043C 9.01e-102
great YAL014C YAL014C 0.0
...
</code></pre>
