...
<li>`-out <output_file>`</li>
<ul style="list-style-type:none;"><li>Write the output to `<output_file>` as opposed to the default of [standard output](#standard_output).</li></ul>
...
Running `makeblastdb` on a FASTA file is fairly simple: `makeblastdb -in <fasta_file> -out <database_name> -dbtype <type> -title <title> -parse_seqids`, where `<type>` is one of `prot` or `nucl`, and `<title>` is a human-readable title (enclosed in quotes if necessary). The `-parse_seqids` flag indicates that the sequence IDs from the FASTA file should be included in the database so that they can be used in outputs as well as by other tools like `blastdbcmd` (discussed below).
...
<code>
[coonrod@n0349 ~/apcb/intro/blast]$ <b>wget 'http://sgd-archive.yeastgenome.org/sequence/S288C_reference/orf_protein/orf_trans.fasta.gz' -O orf_trans.fasta.gz</b>
...
[coonrod@n0349 ~/apcb/intro/blast]$ <b>gunzip orf_trans.fasta.gz</b>
[coonrod@n0349 ~/apcb/intro/blast]$ <b>ls</b>
orf_trans.fasta
</code></pre>
...
<pre id=part1-08-makeblastdb
     class="language-txt 
            ...
            linkable-line-numbers">
<code>
[coonrod@n0349 ~/apcb/intro/blast]$ <b>makeblastdb -in orf_trans.fasta -out orf_trans -dbtype prot -title "Yeast Open Reading Frames" -parse_seqids</b>
...
[coonrod@n0349 ~/apcb/intro/blast]$ <b>ls</b>
orf_trans.fasta  orf_trans.pin  orf_trans.pos  orf_trans.ptf
orf_trans.pdb    orf_trans.pjs  orf_trans.pot  orf_trans.pto
orf_trans.phr    orf_trans.pog  orf_trans.psq
</code></pre>
...
<pre id=part1-08-blastp
     class="language-txt 
            ...
            linkable-line-numbers">
<code>
[coonrod@n0349 ~/apcb/intro/blast]$ <b>blastp -query orf_trans.fasta -db orf_trans -max_target_seqs 2 -max_hsps 1 -evalue 1e-6 -outfmt '7 qseqid sseqid length qlen slen qstart qend sstart send evalue' -out yeast_blastp_yeast_top2.txt -num_threads 4</b>
</code></pre>
...
# Fields: query id, subject id, alignment length, query length, subject length, 
# 1 hits found
YAL003W YAL003W 207     207     207     1       207     1       207     3.23e-150
...
</code></pre>

In the output snippet above, YAL005C has an HSP with itself (naturally), but also one with YLL024C. We'll consider basic analyses containing this sort of data — rows and columns stored in text files, interspersed with extraneous lines — in later chapters.
